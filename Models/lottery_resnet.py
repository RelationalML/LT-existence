...
# and https://github.com/ganguli-lab/Synaptic-Flow
...
class Block(nn.Module):
    ...
    def __init__(self, f_in: int, f_out: int, downsample, nonlinear):
    ...
    def forward(self, x):
        #nonlin = F.relu() F.leaky_relu F.tanh
...
class ResNet(nn.Module):
    ...
    def __init__(self, plan, num_classes, dense_classifier, nonlinear):
        ...
        for segment_index, (filters, num_blocks) in enumerate(plan):
            for block_index in range(num_blocks):
                ...
                blocks.append(Block(current_filters, filters, downsample, nonlinear))
    ...
    def forward(self, x):
        out = self.nonlin(self.bn(self.conv(x)))
        out = self.blocks(out)
...
def _resnet(arch, plan, num_classes, dense_classifier, pretrained, nonlinear):
    model = ResNet(plan, num_classes, dense_classifier, nonlinear)
    if pretrained:
...
# ResNet Models
def resnet20(input_shape, num_classes, nonlinear, dense_classifier=False, pretrained=False):
    plan = _plan(20, 16)
    return _resnet('resnet20', plan, num_classes, dense_classifier, pretrained, nonlinear)
